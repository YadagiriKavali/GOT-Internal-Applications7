        ...
        public static string DoHttpPost(string url, string postXml, string action)
        {
            ...
            catch (WebException WebEx)
            {
                if (WebExceptionStatus.Timeout == WebEx.Status)
                    LogData.Write("MEESEVA", "WEBREQUEST-Exception", LogMode.Excep, WebEx, string.Format("WebRequestProcess => DoHttpPost - Timeout Exception:{0}", WebEx.Message));
                else
                    LogData.Write("MEESEVA", "WEBREQUEST-Exception", LogMode.Excep, WebEx, string.Format("WebRequestProcess => DoHttpPost - Web Exception:{0}", WebEx.Message));
            }
            catch (Exception ex)
            {
                LogData.Write("MEESEVA", "WEBREQUEST-Exception", LogMode.Excep, ex, string.Format("WebRequestProcess => DoHttpPost - Web Exception:{0}", ex.Message));
            }
            finally
            {
                LogData.Write("MEESEVA", "WEBREQUEST", LogMode.Info, string.Format("WebRequestProcess => DoHttpPost - Action: {0}, URL: {1}, Request: {2}, Reponse: {3}, TimeTaken: {4}", action, url, postXml, response, (DateTime.Now.Ticks - startTime) / 10000));
            }
