$(function () {
    //debugger;
    var param1var = $('#hdn_accessToken').val(); //getQueryVariable("access_token");
    var param2var = $('#hdn_code').val(); //getQueryVariable("code");
        ...
        if (records != null) {
            for (var j = 0; j < records.length; j++) {
                ...
                if (records[j].type == "dir") {
                    ...
                    //if (Array.isArray(records[j].mime)) {
                    //    myTr += '<td valign="top" style="width:' + dgcolWidth[0] + '" onclick="GetPDF(\'' + records[j].uri + '\',\'' + accesstoken + '\',\'' + records[j].name + '\',\'' + records[j].mime[0] + '\',\'' + records[j].issuer + '\')">' + records[j].name + '</td>';
                    //} else
                    myTr += '<td valign="top" style="width:' + dgcolWidth[0] + '" onclick="GetPDF(\'' + records[j].uri + '\',\'' + accesstoken + '\',\'' + records[j].name + '\',\'application/pdf\',\'' + records[j].issuer + '\')">' + records[j].name + '</td>';
                }
                //myTr += '<td valign="top" style="width:' + dgcolWidth[1] + '">' + Math.round(iSizeinKB) + ' kB</td>';
                myTr += '<td valign="top" style="width:' + dgcolWidth[2] + '">' + records[j].date + '</td>';
                myTr += '</tr>';
...
function FireRefresh() {
    if ($("#hdn_RefreshToken").val() != "" && $("#hdn_ExpireTime").val() != "") {
        var expireIn = parseInt($("#hdn_ExpireTime").val());
        setTimeout(function () {
            refreshToken($("#hdn_RefreshToken").val());
        }, (expireIn * 200)); //3600 seconds * 200 seconds=12min
    }
}
function refreshToken(refeshToken) {
    try{
        $.ajax({
            type: "POST",
            url: "Handlers/DGUpload.ashx?action=refresh&refreshtoken=" + refeshToken,
            headers: { 'SESSTOKEN': $("#sesstoken").val() },
            error: function (xhr, ajaxOptions, thrownError) {
                $.unblockUI();
            },
            success: function (msg) {
                ...
                if (msg.length != 0) {
                    ...
                    $('#hdn_accessToken').val(data.access_token);
                    $('#hdn_RefreshToken').val(data.refresh_token);
                    $('#hdn_ExpireTime').val(data.expires_in);
                    FireRefresh();
                }
                else {
                    emptyGrid();
