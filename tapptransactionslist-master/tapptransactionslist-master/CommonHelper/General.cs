//$Author: Anilkumar.ko $
//$Date: 8/27/19 10:36a $
//$Header: /Mobileapps/Operator/Davinci/TS/TAppTransactionsList/TAppTransactionsList/CommonHelper/General.cs 2     8/27/19 10:36a Anilkumar.ko $
//$Modtime: 8/26/19 6:40p $
//$Revision: 2 $
...
namespace CommonHelper
{
    public class General
    {
            ...
            catch (WebException ex)
            {
                using (var reader = new StreamReader(ex.Response.GetResponseStream()))
                {
                    if (reader != null)
                    {
                        ...
                        General.WriteLog("COMPARE_TRANS", "Url:" + strURL + Environment.NewLine + " Error:" + reader.ReadToEnd());
                    }
                    General.WriteLog("COMPARE_TRANS", "Url:" + strURL + Environment.NewLine + " Ex:" + ex.ToStr());
                }
        ...
        public static bool SendMail(string strTo, string strCC, string strBCC, string strMailBody, string strSubject, string strAttachment)
        {
            bool isMailSent = false;
            try
            {
                string strFrom = (General.GetConfigVal("MAIL_4M") != string.Empty ? General.GetConfigVal("MAIL_4M") : "mobilecdrs@imimobile.com");

                if (strSubject == string.Empty)
                    strSubject = General.GetConfigVal("MAIL_SUBJECT");

                using (MailMessage oMail = new MailMessage(strFrom, strTo, strSubject, strMailBody))
                {
                    string strSMTPHost = (General.GetConfigVal("SMTPSERVER") != string.Empty ? General.GetConfigVal("SMTPSERVER") : "203.199.178.220");
                    string strUsrID = (General.GetConfigVal("MAIL_USR") != string.Empty ? General.GetConfigVal("MAIL_USR") : "mobilecdrs");
                    string strPWD = (General.GetConfigVal("MAIL_PWD") != string.Empty ? General.GetConfigVal("MAIL_PWD") : "IMI**789");
                    oMail.IsBodyHtml = true;
                    if (strCC != "")
                        oMail.CC.Add(strCC);
                    if (strBCC != "")
                        oMail.Bcc.Add(strBCC);
                    ...
                    smtp.EnableSsl = true;
                    smtp.Credentials = new System.Net.NetworkCredential(strUsrID, strPWD, "");
                    smtp.Port = General.GetConfigVal("MAIL_PORT") == "" ? 25 : Convert.ToInt32(General.GetConfigVal("MAIL_PORT"));
                    smtp.Send(oMail);
                    oMail.Dispose();
                    isMailSent = true;
                }
            ...
            {
                General.WriteLog("General", "Exception in SendMail==>" + ex.ToStr());
            }
            return isMailSent;
        }
        ...
        public static string GetSHA1Hash(string strInput)
        {
            byte[] encPwd = Encoding.UTF8.GetBytes(strInput);
